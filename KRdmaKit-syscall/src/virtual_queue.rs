...
use crate::bindings::*;
use crate::client::*;
use crate::consts::META_SERVER_GID;
use crate::core::*;
    ...
    fn ioctrl(&mut self, cmd: c_uint, arg: c_ulong) -> c_long {
        ...
        let status = match cmd {
            ...
            lib_r_cmd::RegMRs => {
                ...
                let mr = unsafe {
                    ib_alloc_mr(pd,
                                ib_mr_type::IB_MR_TYPE_MEM_REG,
                                1 + (size / pg_sz) as u32)
                };

                unsafe fn reg_mr(mr: *mut ib_mr, address: u64, size: u64) -> u64 {
                    use KRdmaKit::rust_kernel_rdma_base::bindings::{get_free_page, get_user_pages};
                    use KRdmaKit::rust_kernel_rdma_base::*;
                    ...
                    let ret = get_user_pages(address,
                                             entry_len,
                                             0,
                                             // 0x10000,
                                             page_list,
                    ...
                    let sg = sg_list[0 as usize].as_mut_ptr();

                    unsafe { bd_sg_set_page(sg, *page_list, pg_sz as u32, 0) };
                    ...
                    profile.report(6);
                    bd_free_pages(*page_list, 1);

                    return dma_buf;
                }

                fn normal_req(ctx: &RContext,
                              va: u64, dma_pa: u64, mr: *mut ib_mr) {
                ...
                unsafe fn bench_pin(mr: *mut ib_mr, address: u64, size: u64) -> u64 {
                    use KRdmaKit::rust_kernel_rdma_base::bindings::{
                        bd_put_page, bd_free_pages, get_free_page, get_user_pages,
                    };
                    use KRdmaKit::rust_kernel_rdma_base::*;
                    let mut profile: Profile = Profile::new();
                    let run_cnt = 1;
                    ...
                    for _ in 0..run_cnt {
                        ...
                        let ret = get_user_pages(address,
                                                 ...
                                                 0,
                                                 // 0x10000,
                                                 page_list as *mut *mut page,
                                                 null_mut());
                        profile.tick_record(0);

                        for i in 0..entry_len {
                            let cur_page = (page_list as u64 + i as u64 *
                                core::mem::size_of::<*mut page>() as u64) as *mut *mut page;
                            bd_put_page(*cur_page);
                            // bd_free_pages(*cur_page, 1);
                        }
                    }
                    bd_free_pages(*page_list, 1);
                ...
                let res = unsafe { bench_pin(mr, address, size as u64) };
                // let res = unsafe { reg_mr(mr, address, 4096) };

                // normal_req(ctx, address, res, mr);

                unsafe { ib_dereg_mr(mr) };
...
impl<'a> VQ<'a> {
    ...
    fn query_dct_meta(&self, remote_gid: &str) -> Option<EndPoint> {
        let meta_kv_gid_str = String::from(META_SERVER_GID);
