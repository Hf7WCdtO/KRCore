...
use KRdmaKit::consts::UD_HEADER_SZ;
use KRdmaKit::device::RContext;
use KRdmaKit::mem::{Memory, pa_to_va, RMemPhy, va_to_pa};
use KRdmaKit::qp::{RecvHelper, UD, UDOp};
use KRdmaKit::rust_kernel_rdma_base::*;
use KRdmaKit::rust_kernel_rdma_base::linux_kernel_module::file_operations::SeekFrom::End;
...
impl<'a, const N: usize> RPCClient<'a, N> {
    fn dummy_handler(req_addr: u64, reply_addr: u64) -> u32 {
        ...
        let req: *mut u64 = req_addr as *mut u64;
        unsafe {
            memcpy(
    ...
    pub fn create(ctx: &'a RContext, ud: &'a Arc<UD>, endpoint: &'a EndPoint) -> Self {
        let total = 1024 * 4;
    ...
    pub async fn poll_all(&mut self) {
        ...
        loop {
            ...
            if pop_res.is_none() {
                ...
                // TODO: how to end up this polling task gracefully?
                if timer.get_passed_usec() >= 5 * 1000 * 1000 || !self.listened {
                    break;
                }
            } else {
